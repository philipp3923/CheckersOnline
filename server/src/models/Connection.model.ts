...
export default class ConnectionModel {
  ...
  public async goOnline() {
    ...
    for (const friendship of await this.friendshipService.getFriends(
      this.decryptedToken.id
    )) {
      if (
        friendship.friend === this.decryptedToken.id ||
  ...
  public async addSocket(socket: Socket) {
    ...
    for (let key of Object.keys(this.games)) {
      ...
      if (states[key].white === null || states[key].black === null) {
        states[key] = {
          key: key,
          timeType: states[key].timeType,
          time: states[key].time,
          increment: states[key].increment,
          waiting: true,
        };
      }
      socket.join(key);
    ...
    if (this.decryptedToken.role !== Role.GUEST) {
      socket.send("welcome", {
        friends: await this.friendshipService.getFriends(
          this.decryptedToken.id
        ),
        games: states,
      });
