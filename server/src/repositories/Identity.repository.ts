...
export default class IdentityRepository {
  ...
  public async generateUserID() {
    ...
    while (
      (await this.prismaClient.account.findUnique({
        where: { ext_id: id },
      })) !== null
    ) {
  ...
  public async generateGameID() {
    ...
    while (
      (await this.prismaClient.game.findUnique({ where: { ext_id: id } })) !==
      null
    ) {
  ...
  private generateID() {
    return `${Date.now().toString(36)}${this.generatePad(
      this.generateRandom(0, 2176782336).toString(36),
      6
    )}`.toUpperCase();
  }
