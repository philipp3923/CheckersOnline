...
export default class GameRepository {
  ...
  public async saveGame(
    ...
    await this.prismaClient.game.create({
      data: {
        ext_id: id,
        black: { connect: { ext_id: black } },
        white: { connect: { ext_id: white } },
        type: GameType[type],
        time_type: timeType,
        time_limit: time,
  ...
  public async savePlay(
    ...
    //@ts-ignore
    await this.prismaClient.play.create({
      data: {
        index: index,
        timestamp: time,
        time_left: time_left,
  ...
  public async finishGame(id: string, color: Color) {
    ...
    await this.prismaClient.game.update({
      data: { winner: Color[color] },
      where: { ext_id: id },
    });
  }

  public async getActiveGames() {
    return await this.prismaClient.game.findMany({
      where: { winner: null },
      include: { plays: { orderBy: { index: "asc" } } },
    });
  }
