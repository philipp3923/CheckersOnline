...
export default class FriendshipRepository {
  ...
  public async requestFriend(user_id_ext: string, friend_id_ext: string) {
    ...
    if (
      (await this.getFriend(user.id, friend.id)).length > 0 ||
      (await this.getFriend(friend.id, user.id)).length > 0
    ) {
  ...
  public async deleteFriend(user_id_ext: string, friend_id_ext: string) {
    ...
    await this.prismaClient.friend.deleteMany({
      where: { AND: [{ id_user: friend.id }, { id_friend: user.id }] },
    });
  }
    ...
    return await this.prismaClient.friend.findMany({
      where: {
        OR: [
          { id_user: user.id },
          {
            AND: [{ id_friend: user.id }, { type: "REQUEST" }],
          },
        ],
      },
      include: {
  ...
  public async deleteAllFriendships(user_id_ext: string) {
    const user = await this.userRepository.getByAccountID(user_id_ext);
    ...
    await this.prismaClient.friend.deleteMany({
      where: { OR: [{ id_user: user.id }, { id_friend: user.id }] },
    });
  }
}
