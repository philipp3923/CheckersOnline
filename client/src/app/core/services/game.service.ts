...
export class GameService {
  ...
  private games_subjects: {
    [key: string]: Subject<GameStateModel | WaitingStateModel>;
  };
  private gamesSubject: Subject<{
    [key: string]: GameStateModel | WaitingStateModel;
  }>;
  ...
  constructor(private socketService: SocketService) {
    ...
    this.gamesSubject = new Subject<{
      [p: string]: GameStateModel | WaitingStateModel;
    }>();
    this.socketService.addGameStateListener((gameState: GameStateModel) =>
      this.updateGameState(gameState)
    );
    this.socketService.addWelcomeListener((res: any) => this.load(res.games));
    this.socketService.addGameLeaveListener((res: any) =>
      this.removeGame(res.key)
    );
  }

  public addWaitingGame(
    key: string,
    timeType: number,
    time: number,
    increment: number
    ...
    this.games_subjects[key] = new Subject<
      GameStateModel | WaitingStateModel
    >();
    this.games_subjects[key].next(this.games[key]);
    this.gamesSubject.next(this.games);
  }

  public getGamesObserver(): Observable<{
    [key: string]: GameStateModel | WaitingStateModel;
  }> {
    return this.gamesSubject.asObservable();
  }

  public getGameObserver(
    key: string
  ): Observable<GameStateModel | WaitingStateModel> | null {
  ...
  private updateGameState(gameState: GameStateModel) {
    ...
    if (!this.games_subjects[gameState.key]) {
      this.games_subjects[gameState.key] = new Subject<
        GameStateModel | WaitingStateModel
      >();
    }
