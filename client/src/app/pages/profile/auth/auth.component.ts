...
export class AuthComponent implements OnInit {
  ...
  @ViewChild('password_input') passwordInput:
  ...
  isLoginInputValid() {
    return (
      (this.passwordInput?.nativeElement.value.length ?? 0 > 0) &&
  ...
  async register() {
    ...
    try {
      const reg_res = await this.apiService.registerUser(
        email,
        username,
        password
      );
      ...
      this.messageService.addMessage(
        MessageType.ERROR,
        'Username or E-Mail is already in use.'
      );
    }
  }
