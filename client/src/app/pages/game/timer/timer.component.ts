...
export class TimerComponent implements OnInit, AfterContentInit {
  ...
  public setTime(timeInMilliseconds: number) {
    ...
    const minutes =
      Math.floor(timeInMilliseconds / 60000) - hours * 60 - days * 24 * 60;
    const seconds =
      Math.floor(timeInMilliseconds / 1000) -
      minutes * 60 -
      hours * 3600 -
      days * 24 * 3600;
    ...
    if (hours > 0) {
      this.time =
        TimerComponent.addZeroPadding(hours) +
        ':' +
        TimerComponent.addZeroPadding(minutes) +
        ':' +
        TimerComponent.addZeroPadding(seconds);
      return;
    }
    ...
    this.time =
      ...
      TimerComponent.addZeroPadding(seconds) +
      ',' +
      TimerComponent.addZeroPadding(tenthOfSeconds);
  }
