...
export class PlayComponent implements OnInit {
  ...
  @ViewChild('time_slider') public timeSlider:
    | ElementRef<HTMLInputElement>
    | undefined;
  @ViewChild('increment_slider') public incrementSlider:
    | ElementRef<HTMLInputElement>
    | undefined;
  @ViewChild('game_key') public game_keyInput:
    | ElementRef<HTMLInputElement>
    | undefined;
  ...
  switchType(type: number) {
    switch (type) {
      case 0:
        ...
        //@ts-ignore
        if (
          this.timeSlider.nativeElement.value >=
          this.STAT_TIME_MAP.length - 1
        ) {
  ...
  enterGame() {
    ...
    switch (this.state) {
      case 'join_custom':
        ...
        this.socketService.joinCustomGame(key, (res) => {
          if (!res.success) {
            if (res.error === 'Game does not exist') {
              this.messageService.addMessage(
                MessageType.ERROR,
                'A Game with the given Key does not exist.'
              );
            } else if (
              res.error === 'Exceeded amount of possible simultaneous games'
            ) {
              this.messageService.addMessage(
                MessageType.ERROR,
                'You have reached the maximum amount of simultaneous' +
                  ' games (20).'
              );
            } else {
              this.messageService.addMessage(
        ...
        break;
      case 'join_casual':
        this.socketService.joinCasualGame(
          this.type === 1,
          this.time,
          this.increment,
          (res) => {
            if (!res.success) {
              this.messageService.addMessage(
                MessageType.ERROR,
                'We are sorry, this feature has yet to be implemented.'
              );
              return;
            }
        ...
        break;
      case 'create_custom':
        this.socketService.createCustomGame(
          this.type === 1,
          this.time,
          this.increment,
          (res) => {
